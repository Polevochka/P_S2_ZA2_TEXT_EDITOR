...
begin
  ...
  // Мы предполагаем, что в списке есть только ОДИН такой студент, для которого
  // совпадают группа и фамилия
  // поэтому обходим все строки текстового редактора, до тех пока не найдём судента
  // и потом ДОСРОЧНО выходим из него

  // в цикле мы находим индекс строки которую надо удалить
  // если в цикле не найдём такую строку, то в конце i = Lines.Count-1
  // Но так же если нужный студент в последней строке то i = Lines.Count-1
  // Следовательно надо ввести переменную, куда будем сохранять индекс УДАЛЯЕМОЙ
  // строки i_del
  // Начальное значение её пусть будет -1
  // так как переменная i может принимать значения только от 0 до Lines.Count-1
  // следовательно если после выхода из цикла i_del = -1, то студента мы не нашли
  i_del := -1;
  // Чтобы каждый раз не писать Memo1 используем with
  with Memo1 do
  begin
    // Обходим строки
    for i:= 0 to Lines.Count-1 do
    begin
      // Извлекаем из i-ой строки ФАМИЛИЮ студента
      // под неё выделяется пространство 12 символов начиная с 4-ого
      tmpNameStud := copy(Lines[i], 4, 12);
      DelSpace(tmpNameStud); // Фамилия может занимать не польностью 12 символов

      // Извлекаем из i-ой строки ГРУППУ студента
      // под неё выделяется 8 символов начиная с 17-го
      tmpNameGr := copy(Lines[i], 17, 8);
      DelSpace(tmpNameGr); // Так же как и с фамилией(не все символы из 8 есть фамилия)

      // Если фамилия и группа совпадают с оригинальными, то можно выходить из цикла
      if (OrigNameStud = tmpNameStud) and (OrigNameGr = tmpNameGr) then
      begin
        i_del := i;
        break;
      end;
    ...
    // Проверяем если такого студента нет в списке, то выводим сообщение
    if (i_del = -1) then
      ShowMessage('Такого студента в списке нет')
    else
      begin
        // Иначе удаляем строку с таким студентом
        // Для этого у Memo1.Lines есть полезный метод
        Lines.Delete(i);
      end;
