...
var
    ...
    rab_f: textfile; // рабочий файл
    s: string; // строка с информацией о студенте
    Gr: string; // имя группы - из исходного файла
    tmpGr: string; // Имя группы считываемой из файла групп
    isGroupInside: boolean; // Логическая переменная - сообщающая нам есть ли такая группа в файле групп
    sred: real; // средний бал для группы
    count: integer; // Счётчик студентов в ОДНОЙ группе
    o1,o2,o3 : integer; // оценки студента
    res : string; // строка результата
begin
  ...
  begin
    ...
    while (not EOF(rab_f)) do
    begin
      readln(rab_f, tmpGr);
      if (tmpGr = Gr) then
      begin
        isGroupInside := True; // Значит такое имя группы есть файле
  ...
  // Теперь у нас есть файл с Группами студентов
  // можно искать уже средние оценки для групп

  // Средние баллы по группами мы будем выводить в мемо1
  // перед этим его надо очистить
  Memo1.Lines.Clear;

  // Открываем РАБОЧИЙ(с группами) файлы на чтение
  Reset(rab_f);
  // Обходим все группы и для каждой находим средний бал
  While(not EOF(rab_f)) do
  begin
    readln(rab_f, tmpGr);

    // по умолчанию ставим средний бал = 0
    sred := 0;
    // По умолчанию число студентов в группе тоже равно нулю
    count := 0;
    ...
    begin
      ...
      // если группы совпадают то
      if(Gr = tmpGr) then
      begin
        // Увеличиваем счётчик студентов в группе
        count := count + 1;
        // извлекаем оценки этого студента
        o1 := StrToInt(copy(s, length(s)-4, 1)); // 4 позиция с конца 1 символ
        o2 := StrToInt(copy(s, length(s)-2, 1));
        o3 := StrToInt(copy(s, length(s), 1));
        // Суммируем их со средним
        sred := sred + o1 + o2 + o3;
      end;
    end;
    // Обошли файл студентов - надо его закрть
    CloseFile(f);

    // После этого цикла у нас в sred храниться сумма оценок count студентов
    // Среднее ариметическое - это сумма всех оценок на их количество
    sred := sred / (count * 3); //  умножаем на 3 тк у каждого студента по 3 оценки

    // Теперь нужно собрать строку где будет имя группы и средний бал
    // FloatToStrF преобразует вещественное число в строку
    // 3 в конце - число знаков после запятой
    res := tmpGr + '   :' +  FloatToStrF(sred, fffixed, 1, 3);

    // Добавляем получившуюся строку Memo1
    Memo1.Lines.Append(res);
  end;
  // Мы обошли весь файл групп - надо его закрыть
  closeFile(rab_f);

  // Теперь его можно удолить
  DeleteFile('Groups.txt');

  // Мы что-то записывали в мемо, но сам файл не изменяли
  Memo1.Modified := False;
