...
begin
  ...
  begin
    ...
    if (OrigNameStud <> tmpNameStud) or (OrigNameGr <> tmpNameGr) then
      writeln(tmp_f, s); // записываем такого студента во ВРЕМЕННЫЙ файл
  end;
  ...
  closefile(tmp_f); // закрываем временный

  // открываем на ЗАПИСЬ исходный(удаляется всё что было в нём)
  Rewrite(f);
  // открываем на ЧТЕНИЕ исходный
  Reset(tmp_f);

  // обходим ВРЕМЕННЫЙ файл
  while(not EOF(tmp_f)) do
  begin
    readln(tmp_f, s); // считываем одного студента из ВРЕМЕННОГО
    writeln(f, s); // записываем его в ИСХОДНЫЙ файл
  end;
