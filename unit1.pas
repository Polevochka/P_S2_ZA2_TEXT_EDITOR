...
var OrigNameStud: string; // Фамилия студента
    ...
    new_f: textfile; // новый файл, куда будем записывать студентов с обработкой
    name_new_f: string; // имя нового файла
begin
  ...
  // Делаем имя нового файла уникальным, чтобы он не совпадал с другими
  ...
  name_new_f := copy(sf, 1, length(sf)-4);
  // Добавим что-нибудь чтобы наш НОВЫЙ файл отличалься от старого(+ расширение)
  name_new_f := name_new_f + '_NEW_.txt';
  // открываем НОВЫЙ файл на ЗАПИСЬ
  AssignFile(new_f, name_new_f);
  Rewrite(new_f);
  ...
  begin
    ...
    if (OrigNameStud <> tmpNameStud) or (OrigNameGr <> tmpNameGr) then
      writeln(new_f, s); // записываем такого студента во ВРЕМЕННЫЙ файл
  end;

  // после завершения цикла мы собрали ВРЕМЕННЫЙ файл БЕЗ студента(ов) из
  // указанной группы и с указанной фамилией пользователем
  // надо из ВРЕМЕННОГО файла перезаписить студентов в исходный

  // сначала надо закрыть оба файла
  closefile(f); // закрываем исходный
  closefile(new_f); // закрываем новый - уже обработанный файл

  // удаляем СТАРЫЙ(исходный) файл - он нам больше не нужен
  DeleteFile(sf);

  // изменим имя нового файла, на имя СТАРОГО
  RenameFile(name_new_f, sf);
